      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            {selectedIssue && (
              <span>
                问题 #{selectedIssue.number}：{selectedIssue.title}
              </span>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        {investigationStatus.phase === 'idle' ? (
          <div className="space-y-4 flex-1 min-h-0 flex flex-col">
            <p className="text-sm text-muted-foreground">
              从此 GitHub 问题创建任务。任务将添加到你的看板的待办列。
            </p>
            ...
            {loadingComments ? (
              ...
              <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-4">
                <p className="text-sm text-destructive font-medium">评论加载失败</p>
              ...
              <div className="space-y-2 flex-1 min-h-0 flex flex-col">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium flex items-center gap-2">
                    ...
                    选择要包含的评论（{selectedCommentIds.length}/{comments.length}）
                  </h4>
                  <Button
                    variant="ghost"
                    size="sm"
                    ...
                    className="text-xs"
                  >
                    {selectedCommentIds.length === comments.length ? '取消全选' : '全选'}
                  </Button>
                </div>
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-4">
                <h4 className="text-sm font-medium mb-2">任务将包含：</h4>
